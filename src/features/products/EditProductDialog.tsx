...
import type { Product, ProductsResponse, UpdateProductPayload } from "./products.api";
import { useToast } from "../../components/toast/ToastContext";

const schema = z.object({
  title: z.string().min(2, "Title is required"),
  // ✅ keep it number (no coerce) to avoid Resolver<unknown> mismatch
...
export default function EditProductDialog({
  ...
  const mutation = useMutation({
    mutationFn: (args: { id: number; payload: UpdateProductPayload }) => updateProduct(args),
    onSuccess: (updated: Product) => {
      qc.setQueryData(queryKeyForCurrentPage, (old: unknown) => {
        ...
        return {
          ...o,
          products: o.products.map((p) => (p.id === updated.id ? { ...p, ...updated } : p)),
      ...
      push({
        ...
        message: `"${updated.title}" saved successfully.`,
      });

      onClose();
    },
    onError: (err) => {
      push({ type: "error", title: "Update failed", message: parseApiError(err) });
    },
  });
  ...
  useEffect(() => {
    if (open && product) reset(defaultValues);
    if (!open) resetMutation();
  }, [open, product, reset, resetMutation, defaultValues]);
  ...
  const onSubmit: SubmitHandler<FormValues> = (values) => {
    ...
    mutation.mutate({
      ...
      payload: {
        ...
        // keep current image (your dialog doesn't edit it)
                ...
                <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                  <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                    <div className="sm:col-span-2">
                      ...
                      <input
                        {...register("title")}
                        className="mt-2 w-full rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-sm text-white outline-none transition-all focus:border-blue-500/50 focus:bg-slate-800 focus:ring-4 focus:ring-blue-500/10"
                      />
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        step="0.01"
                        // ✅ ensures RHF provides a number (so z.number() passes)
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea
                        ...
                        className="mt-2 w-full resize-none rounded-xl border border-slate-700 bg-slate-800/50 px-4 py-3 text-sm text-white outline-none transition-all focus:border-blue-500/50 focus:bg-slate-800 focus:ring-4 focus:ring-blue-500/10"
                      />
